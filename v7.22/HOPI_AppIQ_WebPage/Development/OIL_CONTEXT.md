...
`_i18n.js` ř.789: `document.documentElement.lang = _lang;`
...
- Breakpoint checker: zobrazí aktivní `@media` pravidla pro každý viewport
...
| AIQ-00112 | AI AppIQ DocWriter | Dokumentační asistent (spec, changelog, CS/EN) | MED | M | 2. |
| AIQ-00110 | AI AppIQ Admin | Administrátorský asistent (OIL, deploy, release) | MED | M | 3. |
| AIQ-00109 | AI AppIQ Tester | Testovací asistent (testcases, OIL testy, UAT) | MED | M | 4. |
...
**Role:** Asistent pro psaní dokumentace platformy AppIQ.
**Co umí:**
...
- Dodržuje ARCH_MAP kódy a naming conventions
...
**Tón:** Systematický, detailní, checklistový
**Kdy nasadit:** Týden 3
...
**Co umí:**
- Generuje HTML/CSS/JS kód v AppIQ stylu a konvencích
...
**2. Status bar**
- Tenký zelený pruh pod headerem
...
- Verze hint (načte se z versions.json background fetch)
...
- Inline SVG, bez externích souborů

### PROMO banner — 5 úrovní

Zobrazuje se pod status barem. Čte `promoLevel`, `promoText`, `promoTextEn` z `versions.json` (latest verze).
...
| Level 1 | Soft ping | 1× sine 880Hz |
| Level 2 | Double ping | 2× sine ascending |
...
- Obsahuje: velký ikona, PROMO label, velký text (Variant B), CTA odkaz na Hub
...
- Soubor: `og-image.png` (1200×630 px, 90374 B)
...
- `_overlayDone` zůstal `false` navždy
...
- GitHub Pages custom domain nastavení
...
- Pro L4/L5: `iconHtml` je orbital SVG místo emoji div
...
**Stav ZAPNUTO** zobrazí fullscreen overlay s:
...
- `#music-toggle` tlačítko: `display:none` v HTML, zobrazí se až po dismiss promo
...
- Odkaz seznam: Worker načte hopiholding.eu, extrahuje href, fetchuje i dcery

### Technická architektura
- KV namespace: `HOPIQ_KB` (vytvoří David v Cloudflare dashboard)
- Cron trigger: `0 6 * * *` (každý den v 6:00 UTC)
...
2. Claude upraví Worker kód: cron handler + scraping funkce + KV read v message handleru
...
### Test checklist
- [ ] Hub karta zobrazena, odkaz funkční
- [ ] Záložky přepínají správně
- [ ] Chatbot konzole odesílá dotazy a vrací odpovědi
- [ ] Knowledge Base záložka zobrazuje status KV
- [ ] Nav header konzistentní s ostatními centry
- [ ] Dark theme, logo, _hopiq.js widget funkční
...
| Effort | taskType | Doporučený model | Cena input/1M |
|--------|----------|-----------------|---------------|
