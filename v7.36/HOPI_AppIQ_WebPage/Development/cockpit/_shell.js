...
const Shell = (() => {
  ...
  const PASS = 'HOPI2026';
  const SHELL_VER = 'v7.36'; // auto-updated by DO_DEPLOY.ps1 // auto-updated by DO_DEPLOY.ps1
  ...
  function renderShell(cfg) {
    ...
    const backLabel = _fromPH
      ? '← Product Hub'
      : (getLang() === 'cs' ? '← Zpět na Cockpit' : '← Back to Cockpit');
    ...
    shell.innerHTML = `
      <div class="c-topbar">
        <div style="display:flex;align-items:center;gap:14px;">
          <a class="c-back" href="${resolvedBackHref}">${backLabel}</a>
