...
## DISCUSSION LOG — Totální audit log (platí vždy, bez výjimky, AIQ-00341)

> Strategické rozhodnutí David Gogela 2026-04-22. **Žádná diskuse nezůstane nezapsaná.**
> Soubor: `DISCUSSION_LOG.md` (root projektu) + `_SESSION_START/DISCUSSION_LOG.md` (archiv)

### Princip — Total Recall

Každé rozhodnutí, akce, poznatek, riziko nebo otázka z jakékoliv diskuse → IHNED zapsat.
Zpětné záznamy jsou povoleny — označit příznakem `[~RETRO]`.
Nic se nevyhazuje — pokud bylo rozhodnutí přepsáno, status = `SUPERSEDED`, nové = `CONFIRMED`.

### ID konvence

```
DISC-YYMMDD-NNN
Příklad: DISC-260422-007
         ↑ rok  ↑ den  ↑ pořadí v rámci dne (001, 002, ...)
```

### Povinná pole každého záznamu

| Pole | Povinné | Hodnoty |
|------|---------|---------|
| `id` | ✅ | `DISC-YYMMDD-NNN` |
| `datum` | ✅ | `YYYY-MM-DD` |
| `čas` | ✅ | `HH:MM` nebo `~HH:MM` (přibližný) nebo `[~RETRO]` |
...
| `participants` | ✅ | Array jmen — viz tabulka níže |
| `output` | ✅ | Array tagů — viz tabulka níže |
| `text` | ✅ | Popis rozhodnutí/akce/poznatku — **tučně klíčové slovo** |
| `kde` | ✅ | Reference: AIQ-NNNNN, BIZ-NNNNN, soubor |
| `assignee` | jen pro ACTION | Kdo zodpovídá za splnění |
| `superseded_by` | jen pro SUPERSEDED | DISC-ID nového rozhodnutí |

### Identifikace účastníků

| Kód | Celé jméno | Role |
|-----|-----------|------|
| `DG` | David Gogela | Product Owner, rozhodovatel |
| `AI` | Claude (claude-sonnet-4-6) | AI developer & architect |
| `HOPI-MGMT` | HOPI Management | HOPI Group leadership |
| `HOPI-IT` | HOPI IT Department | HOPI interní IT |
| `ARTIN` | ARTIN s.r.o. | Technology partner |
| `INTECS` | INTECS | Technology partner |
| `ANTHROPIC` | Anthropic | Strategic partner (budoucí) |

Formát v záznamu: `participants: [DG, AI]` nebo `participants: [DG, AI, HOPI-MGMT]`
**Facilitátor** (kdo vedl diskusi) = první v poli.
**Rozhodovatel** pro typ `DECISION` = vždy `DG` (David Gogela), jinak explicitně uvést.

### Typ záznamu — kdy použít

| Typ | Kdy zapsat |
|-----|-----------|
| `DECISION` | Závazné rozhodnutí — architektura, ceník, produkt, proces. Vždy má `CONFIRMED` nebo `OPEN`. |
| `ACTION` | Úkol/akce vyplývající z diskuse. Musí mít `assignee`. Při splnění → `DONE`. |
| `INSIGHT` | Důležitý poznatek, objev, hypotéza. Nezávazné, ale hodnotné. |
| `RISK` | Identifikované riziko. Má `OPEN` dokud není mitigováno. |
| `QUESTION` | Otevřená otázka, na kterou zatím nemáme odpověď. |
| `NOTE` | Kontextová poznámka — background, vysvětlení, odkaz. |

### Output tagy — pro které publikum

| Tag | Kdy použít |
|-----|-----------|
| `INVESTOR` | Jde do investor prezentace nebo pitch decku |
| `ANTHROPIC` | Jde do Anthropic outreach/partnership materiálů |
| `CEO_BRIEF` | Jde do denního CEO Briefu |
| `HOPI_MGMT` | Jde do HOPI Group management reportů |
| `INTERNAL` | Pouze interní — Claude + David, nesdílet |
| `PUBLIC` | Může jít na web / marketing |

### Strukturované výstupy — jak generovat

Z DISCUSSION_LOG vyfiltrovat:

```
Zápis z meetingu     = date=TODAY, chronologicky, type=DECISION→ACTION→RISK→QUESTION
Investor report      = output=INVESTOR + priority=CRITICAL/HIGH
Anthropic deck       = output=ANTHROPIC, chronologicky
CEO Brief update     = output=CEO_BRIEF, date=TODAY
Open action items    = type=ACTION + status=OPEN/DONE
Open risks           = type=RISK + status=OPEN
Superseded decisions = status=SUPERSEDED → co bylo přepsáno a čím
Decision audit trail = type=DECISION, všechna data, per domain
```

### Vazba na ostatní systémy

| Typ záznamu | Povinná vazba |
|-------------|-------------|
| `DECISION` + `domain=BUSINESS` | → záznam v `BKONTEXT.md` |
| `DECISION` + `domain=ARCHITECTURE/PRODUCT/DEPLOY` | → záznam v `OIL_CONTEXT.md` nebo `BKONTEXT.md` |
| `ACTION` | → záznam v `OIL.json` (AIQ) nebo `BOIL.json` (BIZ) |
| `RISK` | → záznam v `BKONTEXT.md` nebo `OIL_CONTEXT.md` |

### Trigger pro zápis — IHNED po vzniku

1. David řekne nebo napíše rozhodnutí → Claude zapíše DISC-záznam PŘED dalším krokem
2. Vznikne akce → Claude zapíše ACTION záznam + AIQ (STOP pravidlo)
3. Konec session → Claude zkontroluje: jsou všechna dnešní rozhodnutí v logu?
4. Začátek session → Claude zkontroluje DISCUSSION_LOG pro minulou session: chybí něco?
...
## PROAKTIVNÍ ARCHITEKTURA — pravidlo (platí vždy, bez výjimky)

> Strategické rozhodnutí David Gogela 2026-04-22. AIQ-00343.
> **David = business user / rozhodovatel. Claude = IT odborník / architekt.**

**Claude vždy navrhuje architektonicky nejlepší a metodicky nejsprávnější řešení — bez čekání na Davidovo zadání.**

- **Pokud existuje lepší přístup** než co David popsal → navrhni ho. Krátce vysvětli proč. Teprve pak implementuj.
- **Nikdy neimplementuj technicky suboptimální řešení** jen proto, že David o lepší nepožádal.
- **Pokud je lepší řešení výrazně komplexnější** → zmiň trade-off a doporuč timeline (teď / v9.x / backlog).
- **Proaktivní upozornění povinné pro:** škálovatelnost, bezpečnost, datová vrstva, výkon, budoucí migrace.

### Příklady

| David říká | Claude navrhne |
|-----------|---------------|
| "embed data do HTML" | "Navrhnu externalizaci do JSON souboru — škálovatelné, snáze udržovatelné" |
| "ulož do localStorage" | "Pro tato data je vhodnější backend DB, ale localStorage stačí pro Phase 1 — přechod plánujeme na v9.x" |
| "přidej column do tabulky" | "Navrhnu normalizovanou strukturu — tabulka začíná narůstat" |
| "zkopíruj kód" | "Extrahuju do sdíleného modulu — DRY pravidlo" |

### Kdy NENAVRHOVAT alternativu
- David explicitně říká "chci toto konkrétní řešení a vím proč" → respektuj a implementuj.
- Trade-off je minimální a změna by zdržela víc než pomohla.
...
## Aktualizační pravidla — Archive verify list (závazné, 2026-04-22, AIQ-00345)

> Strategické rozhodnutí David Gogela. **Archivace musí vždy zachytit 100% souborů. Neznámý soubor = nezachráněný soubor.**

Kdykoli je přidán nový soubor do projektu (HTML stránka, JSON data, JS modul, CSS), **ihned aktualizovat archive verify list** na dvou místech:

1. **`DO_ARCHIVE.ps1` — `$webChecks` pole** → přidat nový řádek `"cesta\k\souboru.ext"`
2. **`AUTO_ARCHIVE.bat` — existence checks** → přidat řádek `if exist ... (echo OK) else (echo chybi)`

### Kdy povinně aktualizovat

| Přibyl | Co přidat |
|--------|-----------|
| Nová cockpit sub-stránka (`cockpit\xyz.html`) | Oba soubory |
| Nový datový soubor (`cockpit\xyz.json`) | Oba soubory |
| Nový JS modul (`_modules\_xyz.js`) | DO_ARCHIVE.ps1 |
| Nový root HTML soubor | DO_ARCHIVE.ps1 |

### Jak zkontrolovat úplnost

```powershell
# Všechny cockpit soubory vs. co je v checklistu
Get-ChildItem "HOPI_AppIQ_WebPage\Development\cockpit\" | Select-Object Name
# Porovnat s $webChecks v DO_ARCHIVE.ps1
```

Pokud při archivaci VERIFY hlásí `CHYBI` → soubor existuje ale nebyl v checklistu. Fix = přidat a spustit znovu.
...
## FONT PRAVIDLO — platí vždy, bez výjimky (závazné, 2026-04-22)

> Strategické rozhodnutí David Gogela. **Nikdy patkové ani technicky odlišné písmo v UI.**

**ZAKÁZÁNO — nikdy nepoužívat:**
- `Courier New` — serif monospace, špatně čitelný
- `Georgia`, `Times New Roman`, `serif` — klasické patky
- `var(--c-font-mono)` / `var(--mono)` pro UI text (labels, nadpisy, popisy, ribbony, stripy, citáty)

**POVOLENO — kdy použít `var(--c-font-mono)` / `var(--mono)`:**
- Code snippety / terminál výstupy

**Výchozí volba pro veškerý UI text:**
```css
font-family: var(--c-font-sans);   /* = 'Segoe UI', system-ui, sans-serif */
font-family: var(--sans);          /* totéž v MANAGEMENT_COCKPIT.html */
```

**Při psaní nového CSS — checklist:**
1. Používám `font-family`? → je to UI text? → `var(--c-font-sans)` / `var(--sans)`
2. Je to kód/ID/číslo? → `var(--c-font-mono)` / `var(--mono)` povoleno
3. Chybí `font-family`? → přidat explicitně, nespoléhat na dědičnost
...
### Pravidlo 6 — Integrity check PŘED každým deployem (závazné, 2026-04-22, AIQ-00344)

> Strategické rozhodnutí David Gogela. **Deploy musí být VŽDY funkční. Nefunkční deploy = nefunkční produkt = nefunkční důvěra.**

**Povinný preflight checklist (Claude provede před každým deployem):**

| # | Co ověřit | Jak |
|---|-----------|-----|
| 1 | Hlavní stránky se otevřou | Otevřít v prohlížeči (file:// nebo Live Server) |
| 2 | JS bez chyb na load | DevTools Console — nula červených chyb |
| 3 | Datové JSON soubory existují a jsou validní | Ověřit přítomnost + parsovatelnost (discussion-log.json, business-model.json, ...) |
| 4 | Navigace mezi stránkami funkční | Kliknout na klíčové linky (cockpit → sub-stránky → zpět) |
| 5 | Nové featury fungují na golden path | Projít hlavní use-case nové funkce |
| 6 | CHANGELOG.md má dnešní datum | Zkontrolovat před spuštěním DO_DEPLOY.ps1 |

**Pořadí uzávěru session (závazné):**
```
1. Integrity check (tento checklist)
2. DO_DEPLOY.ps1
3. _run_archive_now.ps1
```

Pokud integrity check selže → ZASTAV, oprav, znovu zkontroluj. Deploy se nespustí dokud není zelený.
